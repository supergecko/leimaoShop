<template>
	<view class="myOrder" style="height: 100%;">
		<view class="box">
			<QSTabs :current="current_1" :tabs="tabs_1" width="375" activeColor="#42A2EC" @change="change1($event, '1')"/>
		</view>
		<view class="box" v-show="current_1===0">
			<QSTabs :current="current_2" :tabs="tabs_2" width="150" activeColor="#42A2EC" @change="change2($event, '1')"/>
		</view>
		<!-- 全部订单 -->
				...
				<view class="main">
					<view class="time">
						...
						{{item.pay_status==0&&item.order_status==0? '待付款':
						 item.pay_status==1&&item.order_status==1&&item.shipping_time==''? '待发货':
						 item.pay_status==1&&item.order_status==1&&item.shipping_time>0? '已发货':
						 item.pay_status==1&&item.order_status==2&&item.shipping_time>0? '已收货':''}}</text>
					</view>
					<view class="mill" @click="toPage(item.order_id)">
						<image class="img" src="../../static/images/user/test1kj.png" mode=""></image>
						<view class="wrap">
					...
					<view class="price" style="text-align: right;">
						...
						<text class="txt" style="color: rgb(136, 136, 136);font-size:24upx;">雷猫积分抵扣:{{item.thundercat_coin}}</text><br>
		...
		<view v-show="current_2===2&&current_1===0">
			<view class="content" v-for="(item,index) in undelivery" :key='index'>
				<view class="main">
					<view class="time">
						<text class="txt">{{item.buy_time}}</text>
		...
		<!-- 已发货 -->
		<view v-show="current_2===3&&current_1===0">
			<view class="content" v-for="(item,index) in delivery" :key='index'>
				<view class="main">
					<view class="time">
						<text class="txt">{{item.buy_time}}</text>
		...
		<!-- 已完成 -->
		<view v-show="current_2===4&&current_1===0">
		...
		<!-- 电费订单 -->
		<view v-show="current_1===1" style="background: rgb(245,245,245);min-height: 100%;padding-top: 24upx;">
			<view v-for="(item,index) in electricity_list" :key='index'>
				<view class="ElectricityItem">
					<view style="margin-bottom: 30upx;">
						<text class="ElectricityItemLeftText" style="margin-right: 156upx;">订单号</text>
						<text class="ElectricityItemRightText">{{item.ele_sn}}</text>
					</view>
					<view style="margin-bottom: 30upx;">
						<text class="ElectricityItemLeftText" style="margin-right: 125upx;">订单数量</text>
						<text class="ElectricityItemRightText">{{item.goods_num}}台</text>
					</view>
					<view style="margin-bottom: 30upx;">
						<text class="ElectricityItemLeftText" style="margin-right: 127upx;">电费单价</text>
						<text class="ElectricityItemRightText">{{item.electricity}}元/台/天</text>
					</view>
					<view style="margin-bottom: 30upx;">
						<text class="ElectricityItemLeftText" style="margin-right: 95upx;">当日总电费</text>
						<text class="ElectricityItemRightText">{{item.all_cost}}元</text>
					</view>
					<view>
						<text class="ElectricityItemLeftText" style="margin-right: 64upx;">电费剩余天数</text>
						<text class="ElectricityItemRightText">{{item.all_day}}</text>
						<view class="ElectricityItemBtn" @click="toChargeRecharge(item.order_electricity_id)">立即缴纳</view>
					</view>
				</view>
